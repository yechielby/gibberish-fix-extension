...
extension webviews): cut → detect direction → convert → paste → switch OS
keyboard. This was empirically validated across 5 POC iterations.
...
- `npm test` — run converter unit + integration tests (must stay 20/20)
...
| `src/pipeline.ts` | 11-step orchestration: cut → (select-all fallback) → detect → convert → paste → kb-switch → learn |
